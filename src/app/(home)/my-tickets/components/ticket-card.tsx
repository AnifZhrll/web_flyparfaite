...
type Data = Pick<Ticket, "id"> & {
    flight: Pick<Flight, "departureDate" | "departureCityCode" | "destinationCityCode" | "arrivalDate"> & {
        plane: Airplane
    }
} & {
    ...
    <div className="ticket-card flex justify-between items-center rounded-[20px] p-5 bg-flysha-bg-purple">
            <div className="flex gap-[16px] items-center">
                ...
                <div className="flex flex-col justify-center-center gap-[2px]">
                    <p className="font-bold text-lg  text-white">{data.flight.plane.name}</p>
                    <p className="text-sm text-flysha-off-purple">{data.seat.type}</p>
                </div>
            </div>
            <p className="w-fit h-fit font-bold text-lg  text-white">{dateFormat(data.flight.departureDate, "DD MMM YYYY")}</p>
            <div className="flex items-center gap-[30px]">
                <div className="flex flex-col gap-[2px] text-center">
                    <p className="font-bold text-lg  text-white">{dateFormat(data.flight.departureDate, "HH:mm")}</p>
                    <p className="text-sm text-flysha-off-purple">{data.flight.departureCityCode}</p>
                </div>
                <img src="../assets/images/icons/plane-dotted.svg" alt="icon" />
                <div className="flex flex-col gap-[2px] text-center">
                    <p className="font-bold text-lg  text-white">{dateFormat(data.flight.arrivalDate, "HH:mm")}</p>
